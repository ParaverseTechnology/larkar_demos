...
#define JNI_METHOD(return_type, method_name) \
  JNIEXPORT return_type JNICALL              \
      Java_com_pxy_liblarkvr_VrActivity_##method_name
...
(JNIEnv* env, jobject obj, jobject asset_mgr, jstring appid) {

  const char *c_appid = env->GetStringUTFChars(appid, 0);

  auto app = new ndk_hello_cardboard::HelloCardboardApp(javaVm, obj, asset_mgr, c_appid);
