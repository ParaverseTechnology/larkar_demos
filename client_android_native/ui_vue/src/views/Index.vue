<template>
    <div class="container">
        <van-nav-bar
                title="LarkXR"
                :right-text="$t('message.setUpPage.title')"
        ...
        <van-dropdown-menu>
            <van-dropdown-item v-model="currentAppliType" :options="category" @change="onFresh"/>
                ...
                <van-grid :column-num="2" :gutter="10" :border="false" :center="false">
                    ...
                    >
                        <div class="cover">
                            ...
                            <div v-else-if="item.appliType == 6 || item.appliType == 5 || item.appliType == 3"
                                 class="appli-type">
...
<script >
    ...
    export default {
        ...
        computed: {
            ...
            category(){
                return  [
                    /*   this.$t("message.Index.All")*/
                    {text: this.$t("message.Index.All") , value: "All"},
                    {text: 'AR', value: "AR"},
                    {text: 'SR', value: 'SIM'},
                    {text: 'VR', value: 'VR'},
                ];
            ...
            regionList(){
                return[
                    {
                        //未配置区域
                        text: this.$t("message.Index.regionList0"),
                        value: '',
                        regionId: '',
                    }
                ]
        ...
        methods: {
            async onFresh() {
                try {
                    ...
                    console.warn(e);
                    this.listError = true;
                    /*请求失败，请确认网络连接和服务器地址是否正确*/
                    Toast(this.$t("message.Index.toastMsg0"));
                }
            ...
            async onNextPage() {
                try {
                    ...
                    if (!this.hasNextPage) {
                        /*没有更多啦~*/
                        Toast(this.$t("message.Index.toastMsg1"));
                    }
                } catch (e) {
                    console.warn(e);
                    this.listError = true;
            ...
            freshList(page = 1, clearOld = true) {
                return new Promise((resolve, reject) => {
                    Fetch.Get("getAppliList?appliType=" + this.currentAppliType, {
                        pageSize: 12,
                        page,
                    }).then((res) => {
            ...
            ,
            freshLan() {
               /* const i18n = this.$multiLang;
                store.commit("freshLan", this.currentLan)
                i18n.locale = this.currentLan;*/
                store.commit("freshLan", this.currentLan)
                this.$i18n.locale = this.currentLan;
                this.changeLanData(this.currentLan);
                // 必须增加以下内容，否则语言切换无法刷新。原因未知
                this.$nextTick(() => {
                    ...
                    .then((res) => {
                        if (res.length > 0) {
                            ...
                            for (var i = 0; i < this.regionList.length; i++) {
                                this.regionList[i].text = this.regionList[i].regionName ? this.regionList[i].regionName : this.$t("message.Index.localServer");
            ...
            onEnterAppli(event) {
                ...
                if (appliType == 9 || appliType == 11) {
                    if (!Native.enableCloudXR() && appliType == 11) {
                      /*  当前原生编译库未支持CloudXR.*/
                        Toast(this.$t("message.Index.toastMsg2"));
                        return;
                    }
                    if (!this.isARReady) {
                       /* AR环境未准备好，当前只支持SR应用*/
                        Toast(this.$t("message.Index.toastMsg3"));
                        return;
                    }
                    ...
                    Fetch.Get("taskInfo/getRunningCnt", {appliId})
                        .then((res) => {
                            ...
                            if (parseInt(res.total) <= instanceMax) {
                                ...
                                /*渲染实例不足*/
                                Toast(this.$t("message.Index.toastMsg4"));
                            }
                        })
                        .catch((e) => {
